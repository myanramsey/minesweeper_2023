#include <SFML/Graphics.hpp>
#include <iostream>
#include <cmath>

#include "GUI.h"
#include "Board.h"

#pragma once
using namespace std;

int main(){
    // _________________Creating Board _________________//
    Board board("files/board_config.cfg");
    string player_name;
    // _________________Loading Textures & Font _________________//
    Textures textures;
    sf::Font font;
    // SFML 3.0: Font loading uses openFromFile
    if (!font.openFromFile("files/font.ttf")) {
        cerr << "Failed to load font!" << endl;
        return 1;
    }

    // _________________Welcome Window _________________//
    // SFML 3.0: VideoMode takes a Vector2u, so we use braces {800, 600}
    sf::RenderWindow window(sf::VideoMode({800, 600}), "Minesweeper");

    // _________________Input Text Varibales _________________//
    sf::String input = "|";
    // SFML 3.0: Text constructor is (Font, String, Size)
    sf::Text input_text(font, input, 18);
    input_text.setFillColor(sf::Color::Yellow);
    input_text.setStyle(sf::Text::Bold);
    setText(input_text, 800.0f / 2.0f, 600.0 / 2.0f - 45);
    // _________________Welcome Varibles_________________//
    int name_check = 0;

    while (window.isOpen()) {
        // SFML 3.0: Event polling returns an optional
        while (const auto event = window.pollEvent()){
            // SFML 3.0: Check event type using .is<Type>()
            if (event->is<sf::Event::Closed>())
                window.close();

            // SFML 3.0: Use getIf for events with data (like TextEntered)
            if (const auto* textEvent = event->getIf<sf::Event::TextEntered>()) {
                if (textEvent->unicode == 8 && input.getSize() != 1) { // Backspace
                    input.erase(input.getSize() - 2, 2);
                    input += "|";
                } else if (input.getSize() < 11 && ((65 <= textEvent->unicode && textEvent->unicode <= 90) or
                                                    (97 <= textEvent->unicode && textEvent->unicode <= 122))) {
                    input.erase(input.getSize() - 1, 1);
                    input += textEvent->unicode;
                    input += "|";
                }
                input_text.setString(input);
                setText(input_text, 800.0f / 2.0f, 600.0 / 2.0f - 45);
                
                if (textEvent->unicode == 10) { // Enter key
                    if (CheckString(input) != "") {
                        player_name = CheckString(input);
                        name_check = 1;
                        window.close();
                    }
                }
            }
        }
        window.clear();
        // Pass font by reference to DrawStartScreen
        DrawStartScreen(window, board, font);
        window.draw(input_text);
        window.display();
    }

    if (name_check == 0)
        return 0;

    // _________________Game Window _________________//
    sf::RenderWindow gamewindow(sf::VideoMode({(unsigned int)board.get_width(), (unsigned int)board.get_height()}), "Minesweeper");
    // _________________Sprites _________________//
    // SFML 3.0: Sprites must be initialized with a texture
    sf::Sprite debug(textures.debug);
    debug.setPosition(sf::Vector2f((board.get_cols() * 32) - 304, 32 * ((board.get_rows()) + 0.5f)));

    sf::Sprite face(textures.face_happy);
    face.setPosition(sf::Vector2f(((board.get_cols() / 2.0f) * 32) - 32, 32 * ((board.get_rows()) + 0.5f)));

    sf::Sprite time_button(textures.pause);
    time_button.setPosition(sf::Vector2f((board.get_cols() * 32) - 240, 32 * ((board.get_rows()) + 0.5f)));

    sf::Sprite leaderboard_button(textures.leaderboard);
    leaderboard_button.setPosition(sf::Vector2f(((board.get_cols()) * 32) - 176, 32 * ((board.get_rows()) + 0.5f)));
    
    // _________________Game Window Varaibles_________________//
    bool game_continue = true;
    bool game_lose = false;
    bool debug_toggle = false;
    bool leaderboard_toggle = false;
    bool pause = false;

    auto start_time = chrono::high_resolution_clock::now();
    auto pause_time = chrono::high_resolution_clock::now();
    auto unpaused_time = chrono::high_resolution_clock::now();

    int total_pause_time = 0;
    int elapsed_time = 0;
    int total_time = 0;

    while (gamewindow.isOpen()) {
        while (const auto event = gamewindow.pollEvent()) {
            if (event->is<sf::Event::Closed>()) {
                gamewindow.close();
            }
            
            if (const auto* mouseEvent = event->getIf<sf::Event::MouseButtonPressed>()) {
                // Gets Coordinates
                auto x = sf::Mouse::getPosition(gamewindow).x;
                auto y = sf::Mouse::getPosition(gamewindow).y;
                cout << x << " : " << y << endl;
                
                if (mouseEvent->button == sf::Mouse::Button::Left) {
                    sf::FloatRect debugBounds = debug.getGlobalBounds();
                    sf::FloatRect faceBounds = face.getGlobalBounds();
                    sf::FloatRect timeBounds = time_button.getGlobalBounds();
                    sf::FloatRect leaderBounds = leaderboard_button.getGlobalBounds();

                    if (y < (board.get_rows() * 32) && game_continue) {
                        x = floor(x / 32);
                        y = floor(y / 32);
                        board.RevealAdjacentCells(x, y);
                        if (board.CheckMineClicked(x, y)) {
                            total_time = 0;
                            face.setTexture(textures.face_lose);
                            game_continue = false;
                            game_lose = true;
                        } else if (board.GameWon()) {
                            Leaderboard_reset();
                            ChangedLeaderBoard(player_name, total_time);
                            leaderboard_toggle = true;
                            game_continue = false;
                        }
                    } else if (debugBounds.contains(sf::Vector2f(x, y)) && game_continue) {
                        debug_toggle = !debug_toggle;
                    } else if (faceBounds.contains(sf::Vector2f(x, y))) {
                        Leaderboard_reset();
                        Board new_board("files/board_config.cfg");
                        board = new_board;

                        start_time = chrono::high_resolution_clock::now();
                        elapsed_time = 0;
                        face.setTexture(textures.face_happy);

                        game_continue = true;
                    } else if (timeBounds.contains(sf::Vector2f(x, y))) {
                        pause = !pause;
                        if (!pause) {
                            pause_time = chrono::high_resolution_clock::now();
                            time_button.setTexture(textures.pause);
                            elapsed_time += chrono::duration_cast<chrono::seconds>(pause_time - unpaused_time).count();
                        } else {
                            time_button.setTexture(textures.play);
                            unpaused_time = chrono::high_resolution_clock::now();
                        }
                    } else if (leaderBounds.contains(sf::Vector2f(x, y))) {
                        leaderboard_toggle = true;
                        pause = !pause;
                        if (!pause) {
                            pause_time = chrono::high_resolution_clock::now();
                            time_button.setTexture(textures.pause);
                            elapsed_time += chrono::duration_cast<chrono::seconds>(pause_time - unpaused_time).count();
                        } else {
                            time_button.setTexture(textures.play);
                            unpaused_time = chrono::high_resolution_clock::now();
                        }
                    }
                }
                else if (mouseEvent->button == sf::Mouse::Button::Right && game_continue) {
                    if (y < (board.get_rows() * 32)) {
                        x = floor(x / 32);
                        y = floor(y / 32);
                        board.FlaggedCell(x, y);
                    }
                }
            }
        }

        gamewindow.clear(sf::Color::White);

        // _________________Time_________________//
        auto game_duration = chrono::duration_cast<chrono::seconds>(chrono::high_resolution_clock::now() - start_time);

        if (!pause && game_continue) {
            total_time = game_duration.count();
            total_time -= elapsed_time;
        }

        DrawTime(gamewindow, board, textures, total_time);
        DrawReavealedTiles(gamewindow, board, textures);

        if (!pause) {
            DrawNumbers(gamewindow, board, textures);
            DrawHidden(gamewindow, board, textures);
            DrawFlagged(gamewindow, board, textures);
            if (debug_toggle)
                DrawDebug(gamewindow, board, textures);
        }

        FlagCount(gamewindow, board, textures);
        gamewindow.draw(leaderboard_button);
        gamewindow.draw(time_button);

        if (!game_continue && game_lose) {
            DrawMines(gamewindow, board, textures);
        }

        gamewindow.draw(debug);
        gamewindow.draw(face);

        gamewindow.display();

        if (leaderboard_toggle) {
            sf::RenderWindow leaderboard_window(sf::VideoMode({(unsigned int)board.leaderboard_width(), (unsigned int)board.leaderboard_height()}),"Leaderboard");
            while (leaderboard_window.isOpen()) {
                while (const auto event = leaderboard_window.pollEvent()) {
                    if (event->is<sf::Event::Closed>()) {
                        leaderboard_window.close();
                        leaderboard_toggle = false;
                        pause = false;
                        if (!pause) {
                            pause_time = chrono::high_resolution_clock::now();
                            time_button.setTexture(textures.pause);
                            elapsed_time += chrono::duration_cast<chrono::seconds>(pause_time - unpaused_time).count();
                        } else {
                            time_button.setTexture(textures.play);
                            unpaused_time = chrono::high_resolution_clock::now();
                        }
                    }
                }
                leaderboard_window.clear(sf::Color::Blue);
                DrawLeaderBoard(leaderboard_window,board,textures,font);
                leaderboard_window.display();
            }
        }
    }
}